...
import com.example.healthapplication.databinding.ActivityMapsBinding
...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback, SensorEventListener {
    ...
    val timeNow: Long = SystemClock.elapsedRealtime()
    ...
    override fun onSensorChanged(sensorEvent: SensorEvent?) {
        ...
        sensorEvent.values.firstOrNull()?.let {
            ...
            val timeElapsed: Long = SystemClock.elapsedRealtime() - timeNow
            updateAllDisplayText(currentNumberOfStepCount,distanceTraveled,caloriesBurnt,timeElapsed)
            Log.d("TAG", "Steps count: $currentNumberOfStepCount ")
            Log.d("TAG", "Elapsed Time: $timeElapsed ")
            val pace = ((timeElapsed/3600000.toDouble()))
            Log.d("TAG", "Elapsed Time: $pace ")
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.locationToggle.setOnCheckedChangeListener(CompoundButton.OnCheckedChangeListener { buttonView, isChecked ->
            if (isChecked) {
                mMap.clear()
                isTracking = true
                updateAllDisplayText(0, 0.0,0,0)
                startTracking()
            } else {
                endButtonClicked()
            }
    ...
    private fun updateAllDisplayText(stepCount: Int, totalDistanceTravelled: Double, caloriesBurnt: Int, timeElapsed: Long) {
        ...
        binding.totalDistanceTextView.text = String.format("Distance: %.2f Miles", totalDistanceTravelled)
        binding.totalCaloriesburntTextView.text = String.format("Calories: %d Cal", caloriesBurnt)

        val averagePace = if (stepCount != 0)  totalDistanceTravelled/(timeElapsed/3600000.toDouble()) else 0.0
        binding.averagePaceTextView.text = String.format("Avg. Pace: %.2f MPH", averagePace)
    }
